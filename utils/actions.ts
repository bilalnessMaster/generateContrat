...
function handleAuthError(error: unknown & {type : string}): string {
...
export const logIn = async (preState: string | undefined, formData: FormData) => {
    try {        
        const validatedata = crendentialsSchema.safeParse({
            email: formData.get("email"),
            password: formData.get('password')
        })

        if (!validatedata.success) {
            return 'donnes invalid'
        }
        const { email, password } = validatedata.data
        console.log(formData, 'data');
        await signIn("credentials", formData)
    } catch (error : any & {type : string} ) {
